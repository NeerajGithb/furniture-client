...
const PRODUCT_HIERARCHY = {
  "sofa": {
    primary: ["sofa", "sectional", "couch", "settee"],
    secondary: ["loveseat", "recliner"],
  ...
  "chair": {
    primary: ["chair"],
    secondary: ["stool", "bench", "armchair"],
    category: ["seating", "office", "dining"],
    attributes: ["material", "color", "style"],
    boost: 2.0,
    avoid: ["sofa", "table", "bed", "cabinet"]
  },
  "table": {
    primary: ["table"],
    secondary: ["desk", "console"],
  ...
  "bed": {
    primary: ["bed"],
    secondary: ["mattress", "headboard", "cot"],
  ...
  "cabinet": {
    primary: ["cabinet", "wardrobe", "cupboard"],
    secondary: ["shelf", "storage", "dresser"],
  ...
  "almirah": {
    primary: ["almirah", "wardrobe", "cupboard", "cabinet"],
    secondary: ["almari", "storage", "closet"],
...
class PureSearchService {
  ...
  static isModifier(token) {
    ...
    if (synonymGroup && ['material_wood', 'material_metal', 'material_fabric'].some(g => 
  ...
  /**
   * Build context-aware search pipeline
   */
  static buildSearchPipeline(tokens, numerics, classified, intent) {
    try {
      ...
      if (tokens.length > 0 || Object.keys(numerics).length > 0) {
        ...
        // Strict filtering based on intent and relevance
        ...
        pipeline.push({
          $match: {
            $and: [
              ...
              {
                $or: [
                  { intentMatch: { $gte: 1 } }, // Must match primary intent
                  { 
                    $and: [
                      { searchScore: { $gt: 8.0 } }, // Or have very high overall score
        ...
        // No search terms - use popularity/recency
        pipeline.push({
          $addFields: {
            searchScore: {
              $add: [
      ...
      // Enhanced sorting with intent priority
      pipeline.push({
        $addFields: {
          sortPriority: {
            $add: [
              // Intent match gets highest priority
              { $multiply: ["$intentMatch", 100] },
              
              // Relevance category priority
              {
                $switch: {
                  branches: [
                    { case: { $eq: ["$relevanceCategory", "exact"] }, then: 50 },
                    { case: { $eq: ["$relevanceCategory", "high"] }, then: 30 },
                    { case: { $eq: ["$relevanceCategory", "medium"] }, then: 15 },
  ...
  static buildContextAwareScore(classified, numerics, intent) {
    try {
        ...
        classified.regular.forEach(token => {
          ...
          scoreComponents.push({
            $multiply: [
              ...
              {
                $cond: [
                  { $ne: [{ $indexOfCP: [{ $toLower: { $ifNull: ["$brand", ""] } }, token] }, -1] },
                  2,
                  0
                ]
              }
            ...
            $multiply: [
              FIELD_WEIGHTS.material,
              {
                $cond: [
                  {
                    $or: [
                      { $ne: [{ $indexOfCP: [{ $toLower: { $ifNull: ["$material", ""] } }, token] }, -1] },
                      { $ne: [{ $indexOfCP: [{ $toLower: { $ifNull: ["$attributes.material", ""] } }, token] }, -1] }
                    ]
                  },
                  3,
                  0
                  ...
                  $and: [
                    { $ne: [{ $indexOfCP: [{ $toLower: "$name" }, token] }, -1] },
                    {
                      $or: classified.primary.concat(classified.regular).map(primaryToken => ({
                        $ne: [{ $indexOfCP: [{ $toLower: "$name" }, primaryToken] }, -1]
                      }))
                    }
                  ]
                },
      ...
      let baseScore = scoreComponents.length > 0 ? 
        { $add: scoreComponents } : 
        { $literal: 0.1 };
  ...
  static async search(query, options = {}) {
    try {
      ...
      try {
        ...
        if (results.length === 0 && (classified.primary.length > 0 || classified.regular.length > 0)) {
          ...
          relaxedPipeline.push({
            $project: {
              // Basic product info
              name: 1, slug: 1, description: 1, metaTitle: 1, metaDescription: 1,
              // Pricing
              finalPrice: 1, originalPrice: 1, discountPercent: 1, emiPrice: 1,
              // Images
              mainImage: 1, galleryImages: 1,
              // Categories & Classification
              categoryId: 1, subCategoryId: 1, category: 1, subcategory: 1,
              // Product attributes
              material: 1, dimensions: 1, weight: 1, colorOptions: 1, size: 1, variants: 1, attributes: 1,
              // Stock & Status
              inStockQuantity: 1, isPublished: 1, isBestSeller: 1, isFeatured: 1, isNewArrival: 1,
              // Branding & Tags
              brand: 1, badge: 1, tags: 1,
              // Reviews & Ratings
              reviews: 1, ratings: 1,
              // Sales & Analytics
              totalSold: 1, viewCount: 1, wishlistCount: 1,
              // Policies & Info
              returnPolicy: 1, warranty: 1, shippingInfo: 1,
              // Timestamps
      ...
      return {
        ...
        fallback: total > 0 && numericTokens.length > 0 && results.some(r => !r.searchScore || r.searchScore < 3)
      };
