...
export const handleAuthError = (input, resBody = {}, setError, isLogin = false) => {
  const firebaseMap = {
    // 🔐 Firebase errors (for both login/signup)
    "auth/invalid-email": "Invalid email address",
    "auth/user-not-found": "No account found with this email",
    "auth/wrong-password": "Incorrect password",
    "auth/email-already-in-use": "This email is already registered",
    "auth/weak-password": "Password must be at least 6 characters",
    "auth/invalid-credential": isLogin ? "Incorrect email or password" : "Signup failed. Please check your details.",
  ...
  if (typeof input === "string") {
    const message = firebaseMap[input] || resBody?.error || "Something went wrong. Please try again.";
    setError(message);
